...
type ImportResult = {
  ...
  errors?: { row: number; matric_number?: string; message: string }[];
  message?: string;
};
...
export function ImportWalletPage() {
  ...
  const loadPending = () => {
    api.get('/api/wallet/import-pending')
  ...
  const downloadTemplate = async () => {
    try {
      const { data } = await api.get('/api/wallet/import-template', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([data]));
      const link = document.createElement('a');
      link.href = url;
      link.download = 'wallet-import-template.xlsx';
  ...
  const pollStatus = async (importId: string) => {
    const { data } = await api.get(`/api/wallet/import/${importId}`);
    setResult(data);
    if (data.status === 'queued' || data.status === 'processing') {
      ...
      message.success(`Posted ${data.posted || 0} wallet row(s).`);
      loadPending();
  ...
  const submitUpload = async () => {
    ...
    setResult(null);
    try {
      const { data } = await api.post('/api/wallet/import', formData, {
      ...
      message.error(err.response?.data?.message || 'Unable to import wallet history.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const downloadErrors = async () => {
    ...
    try {
      const { data } = await api.get(`/api/wallet/import/${result.import_id}/errors`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([data]));
      const link = document.createElement('a');
      link.href = url;
      link.download = 'wallet-import-errors.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-5">
      <WorkspaceHero
        ...
        title="Import wallet history"
        description="Replay credits and debits by matric number in date order. Wallet credit does not count as tuition paid. If a debit would go negative, remaining rows for that matric are skipped."
        icon={Wallet}
      >
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <StatCard label="Posted" value={result?.posted ?? '—'} hint="Written to a wallet" icon={Wallet} />
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-4">
        ...
        <p className="text-sm text-slate-600">
          One row is one credit or debit. Required: matric_number, type (credit/debit), amount, occurred_at.
          Import invoices separately if fees were billed. Wallet history does not settle invoices.
        </p>
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Button type="primary" loading={uploading} onClick={submitUpload}>Import wallet history</Button>
        </div>
        ...
        {result?.status === 'done' && (
          <Alert
            type="success"
            showIcon
            message={`Posted ${result.posted || 0} wallet row(s). ${result.pending || 0} pending. ${result.skipped || 0} skipped.`}
          />
        )}
